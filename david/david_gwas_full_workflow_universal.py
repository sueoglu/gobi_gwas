...
os.makedirs(r"../universal_data/preprocessing", exist_ok=True)
os.makedirs(r"../universal_data/tmp_files", exist_ok=True)
...
subprocess.run([r"plink_win64_20250819/plink.exe",
                ...
                "--hwe", "1e-6",
                "--make-bed",
                "--out", r"../universal_data/preprocessing/chr22_common"])

subprocess.run([r"plink_win64_20250819/plink.exe",
                "--bfile", r"../universal_data/preprocessing/chr22_common",
                "--mind", "0.02",
                "--make-bed",
...
for h2 in h2s:
    for n_c in n_causals:
        ...
        os.makedirs(f"../universal_data/QQ_manhattan/preprocessed_dataset/h2_{h2}/causal_{n_c}", exist_ok=True)

        
        qq_plot(None, pv, idx_caus)
        plt.title(f"QQ plot (n_causals = {n_c} and h2 = {h2})")
        plt.savefig(f"../universal_data/QQ_manhattan/preprocessed_dataset/h2_{h2}/causal_{n_c}/qq_h2_{h2}_causal_{n_c}.png")
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(f"../universal_data/QQ_manhattan/preprocessed_dataset/h2_{h2}/causal_{n_c}/manhattan_h2_{h2}_causal_{n_c}.png")
        plt.close()
        #"""
        ...
        #"""
        os.makedirs(f"../universal_data/QQ_manhattan/train_subset/h2_{h2}/causal_{n_c}", exist_ok=True)

        qq_plot(None, pv, idx_caus)
        plt.title(f"QQ plot (n_causals = {n_c} and h2 = {h2})")
        plt.savefig(f"../universal_data/QQ_manhattan/train_subset/h2_{h2}/causal_{n_c}/qq_h2_{h2}_causal_{n_c}.png")
        plt.close()
        ...
        plt.ylabel('-log$_{10}$ P')
        plt.tight_layout()
        plt.savefig(f"../universal_data/QQ_manhattan/train_subset/h2_{h2}/causal_{n_c}/manhattan_h2_{h2}_causal_{n_c}.png")
        plt.close()
        #"""
        ...
        os.makedirs(f"../universal_data/gwas_results/h2_{h2}/causal_{n_c}", exist_ok=True)
        gwas_df.to_csv(f"../universal_data/gwas_results/h2_{h2}/causal_{n_c}/train_h2_{h2}_causal_{n_c}_gwas_results.txt", sep="\t", index=False)

        os.makedirs(f"../universal_data/clumping/h2_{h2}/causal_{n_c}", exist_ok=True)
        subprocess.run([r"plink_win64_20250819/plink.exe",
                        "--bfile", r"../universal_data/preprocessing/chr22_preprocessed_train_subset",
                        "--clump", f"../universal_data/gwas_results/h2_{h2}/causal_{n_c}/train_h2_{h2}_causal_{n_c}_gwas_results.txt",
                        "--clump-snp-field", "SNP",
                        "--clump-field", "P",
                        "--clump-p1", "1e-4",
                        "--clump-p2", "1e-2",
                        "--clump-r2", "0.07",
                        "--clump-kb", "200",
                        "--out", f"../universal_data/clumping/h2_{h2}/causal_{n_c}/h2_{h2}_causal_{n_c}_clumped"])

        clumped = pd.read_csv(
            f"../universal_data/clumping/h2_{h2}/causal_{n_c}/h2_{h2}_causal_{n_c}_clumped.clumped", sep=r'\s+', engine='python'
            )
